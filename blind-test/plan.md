...
export interface RealtimeChannel {
  ...
  authorizePresence(channel: string, socketId: string, userId: string, userInfo: unknown): { auth: string; channel_data?: string };
}
...
Légende : *(D)* domaine, *(A)* application, *(I)* infrastructure, *(P)* présentation, *(X)* transverse.
